...
from my_pipeline_stable_diffusion import MyStableDiffusionPipeline
...
def main():
    ...
    #!!   
    # pipeline = StableDiffusionPipeline.from_pretrained("runwayml/stable-diffusion-v1-5", torch_dtype=torch.float32).to(device)
    pipeline = MyStableDiffusionPipeline.from_pretrained("runwayml/stable-diffusion-v1-5", torch_dtype=torch.float32).to(device)
    ...
    for i, ref_img in enumerate(loader):
        ...
        if measure_config['operator'] ['name'] == 'inpainting':
            ...
            y_n = y_n.detach()
            #!! resize if latent space model
            # y_n = torch.nn.functional.interpolate(y_n, size=ref_img.shape[-2:])
            mask = None
        # Sampling
