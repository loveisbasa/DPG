...
class MyStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    # #     super(MyStableDiffusionPipeline, self).__init__(*args,**kwargs)
    # #     # super().__init__(*args, **kwargs)
    # #     # *args, **kwargs
        
    # # def __init__( self, cond_fn=None, operator_name = "inpainting"):
    # #     super().__init__()
    ...
    def __call__(
        ...
        # #!! map to latent space if ldm
        # with torch.no_grad():
        #     measurement = self.vae.encode(measurement.to(prompt_embeds.dtype)).latent_dist.sample()
        #     measurement = measurement * self.vae.config.scaling_factor
        #     measurement = measurement.detach()
        # import pdb; pdb.set_trace() #af
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latents_now, pred_xstart = out['prev_sample'], out['pred_original_sample']
                ...
                pred_image = self.vae.decode(pred_xstart / self.vae.config.scaling_factor, return_dict=False)[0]
                #!!
                latents_now, distance = self.cond_fn(x_t=latents_now,
